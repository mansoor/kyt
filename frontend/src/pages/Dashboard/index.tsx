...
export default function DashboardPage() {
  ...
  if (isLoading) {
    return (
      <AppShell>
        <div className="flex items-center justify-center h-64 text-ink/40">Loading…</div>
      </AppShell>
    )
  }
    ...
    return (
      <AppShell>
        <div className="flex flex-col items-center text-center py-24 gap-5 px-6">
          <div className="w-14 h-14 rounded-2xl bg-ink/5 border border-ink/10 flex items-center justify-center">
            <Car className="w-7 h-7 text-ink/30" />
          </div>
          <div>
            <h1 className="text-ink text-xl font-bold mb-1">No vehicles connected</h1>
            <p className="text-ink/50 text-sm">Connect your Tesla account in Settings to start.</p>
          </div>
          <button
  ...
  return (
    <AppShell>
      <div className="max-w-6xl mx-auto px-6 py-6 space-y-6">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
          {vehicles.map((v) => (
            <button
              ...
              className={`glass rounded-2xl p-5 text-left transition-all ${activeCar === v.id ? 'ring-2 ring-brand-blue' : 'hover:bg-ink/10'}`}
            >
              <div className="flex items-center justify-between mb-3">
                <div>
                  <p className="text-ink font-semibold">{v.name ?? v.vin ?? `Car ${v.id}`}</p>
                  <p className="text-ink/40 text-xs">{v.model ?? '—'}</p>
                </div>
                <span className={`w-2.5 h-2.5 rounded-full ${STATE_DOT[v.state] ?? 'bg-ink/10'}`} />
              </div>
              {v.battery_level !== null && (
                <div className="space-y-1">
                  <div className="flex justify-between text-sm">
                    ...
                    <span className="text-ink font-medium">{v.battery_level}%</span>
                  </div>
              ...
              <p className="text-ink/30 text-xs mt-2 capitalize">{v.state}</p>
            </button>
          ))}
        </div>
        ...
        {kpis && (
          <>
            <h2 className="text-ink/50 text-xs font-medium uppercase tracking-wider">Last 7 Days</h2>
        ...
        {battery_history.length > 0 && (
          <div className="glass rounded-2xl p-5">
            <h2 className="text-ink/60 text-sm font-medium mb-4">Battery Level — Last 30 Days</h2>
        ...
        {recent_activity.length > 0 && (
          <div className="glass rounded-2xl p-5">
            <h2 className="text-ink/60 text-sm font-medium mb-4">Recent Activity</h2>
            <div className="space-y-2">
              {recent_activity.map((ev, i) => (
                <button
                  key={i}
                  onClick={() => navigate(ev.type === 'drive' ? `/drives/${ev.id}` : `/charges/${ev.id}`)}
                  className="w-full flex items-center gap-4 px-3 py-2.5 rounded-lg hover:bg-ink/5 transition-colors text-left"
                >
                  ...
                  <div className="flex-1 min-w-0">
                    <p className="text-ink text-sm font-medium capitalize">{ev.type}</p>
                  ...
                  <div className="text-right shrink-0">
                    {ev.type === 'drive' && ev.distance_km != null && (
                      <p className="text-ink text-sm">{ev.distance_km} km</p>
                    )}
                    {ev.type === 'charge' && ev.energy_added_kwh != null && (
                      <p className="text-ink text-sm">{ev.energy_added_kwh} kWh</p>
                    )}
                    {ev.duration_min != null && (
                      <p className="text-ink/40 text-xs flex items-center gap-1 justify-end">
